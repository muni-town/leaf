  ...
  try {
    ...
    events = parsed.map((event, index) => {
      ...
      if (!event.payload || typeof event.payload !== "string") {
        throw new Error(`Event ${index}: missing or invalid 'payload' field (must be base64 string)`);
      }
    ...
    throw new Error(`Failed to parse events file: ${error instanceof Error ? error.message : String(error)}`);
  }

  const client = await createClient(options);

  try {
    await client.sendEvents(streamId, events);

    outputJson({
      success: true,
      message: `Successfully sent ${events.length} event(s) to stream ${streamId}`,
      count: events.length,
    });
  } catch (error) {
